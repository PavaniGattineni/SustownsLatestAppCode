package com.sustowns.sustownsapp.Models;

public class TransportOrdersModelObj {

    private String transport_userid;
    private String manual_automatic;


    public String getTransport_userid() {
        return transport_userid;
    }

    public void setTransport_userid(String transport_userid) {
        this.transport_userid = transport_userid;
    }

    public String getManual_automatic() {
        return manual_automatic;
    }

    public void setManual_automatic(String manual_automatic) {
        this.manual_automatic = manual_automatic;
    }

}
